...
**Where blanks and choices may sit:** never inside the quoted term of a `{{def:}}` (the term must not vary), and in the body never directly after `\`, `&`, `<`, `!`, or `]`, nor directly before `[` or `(` — the inserted text could combine with the template text (Error). Ordinary punctuation after them (`.`, `,`, `;`, `)`) is fine.
...
- `{{def:}}` inside a drafting note; a `{{placeholder:}}` or `{{choose:}}` inside a defined term's quotes, or directly after `\ & < ! ]` or directly before `[` / `(`
